...
見られる統制の例：要件を追加した直後に「テストが対応していない要件」が現れ、それが解消するまで作業を通さない／品質ゲートが「テストの無い要件」や「許可範囲外の変更」で AI を止める／AI が書き込んだ承認を認めない／前の工程の承認前に次の工程が始まったことを順序違反として示す／他社の API 契約を破る変更を止める（架空）
