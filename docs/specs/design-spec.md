...
- **統制ポイント**：あるイベントを適用したとき、ゲートの判定が停止・未達、無効な承認、逸脱の開始、順序違反の発生のいずれかが起きたもの
- **説明文**（`narrate`）：イベントの種類ごとに「いま起きたこと」（見出しと補足）と、Hub が何を統制したか（統制ポイントのとき）を返す。
...
| 中央 | いま起きたこと、選んだ画面 |
| 右 | 出来事の記録（凡例・絞り込み・統制ポイントの強調） |
| 下部 | 再生操作（統制ポイントの位置をスライダー上に印で示す） |
